...
//  javac -d ../../classes MaximumNonAdjacentSubSequenceSum.java
//  java MaximumNonAdjacentSubSequenceSum
...
class Solver {
    ...
    String getMaxSubarraySum(int[] arr, int n)throws Exception
    {
        if(n == 1)return arr[0] + "";
        boolean flag = false;
        for(int i = 0; i < n; i++)if(arr[i] > 0)flag |= true;
        if(!flag)
            hp.println(hp.max(arr));
