...
//  javac -d ../../classes CollectingMangoes.java
//  java CollectingMangoes
...
class Solver {
    ...
    {
        int tcs = hp.nextInt();
        for(int tc = tcs; tc > 0; tc--)
        {
            hp.println("Case " + (tcs - tc + 1) + ":");
            ...
            for(int i = 0; i < n; i++)
            {
                ...
                if(q.equals("A"))
                {
                    int mango = hp.nextInt();
                    st1.push(mango);
                    if(st2.size() == 0)
                    {
                        st2.push(mango);
                    }
                    else if(mango >= st2.peek())
                        st2.push(mango);
                }
                else if(q.equals("R"))
                {
                    /*
                    hp.println("here at case " + (tcs - tc + 1) + " query "
                                + i + " " + st1.size() + " " + st2.size()); */
                    if(!st1.isEmpty())
                    {
                        int k = st1.pop();
