...
//  javac -d ../../classes OracAndLCM.java
//  java OracAndLCM
    ...
    void solve() throws Exception
    {
        //for(int tc = hp.nextInt(); tc > 0; tc--)
        {
            ...
            for(int i = 0; i < n-1; i++)
            {
                ...
                for(int j = i + 1; j < n; j++)
                {
                    hp.println(lcm(arr[i], arr[j]) + " "+ hp.gcd(arr[i], arr[j]));
                }
            }
            */
            ...
            for(int i = 0; i < n - 1; i++)
            {
                lcms[i + 1] = lcm(arr[i], arr[i + 1]);
                gcds[i + 1] = hp.gcd(arr[i], arr[i + 1]);
            }
            hp.println(Arrays.toString(lcms));
            hp.println(Arrays.toString(gcds));
            //hp.println(lcm);
