...
//  javac -d ../../classes SpaceFormula.java
//  java SpaceFormula
...
class Solver {
    ...
    void solve() throws Exception
    {
        int n = hp.nextInt();
        int d = hp.nextInt() - 1;
        int[] prev = hp.getIntArray(n);
        int[] points = hp.getIntArray(n);
        int[] ans = new int[n];
        int maxPossibleByD =  points[0] + prev[d];
        int rank = d, k = 1;
        for(int i = d - 1; i >= 0; i--)
        {
           while(k<n  && prev[i]+points[k] > maxPossibleByD)
           {
               k++;
           }
           if(k>=n)
