...
class Solver {
    ...
    void solve() throws Exception
    {
        ...
        for(int tc = hp.nextInt(); tc > 0; tc--)
        {
            ...
            int[] couldInfectArr = new int[n];
            Arrays.sort(arr);
            for(int i = 0; i < n; i++)
            {
                int couldInfect = 0,toRight = 0, toLeft = 0;
                for(int j = i + 1; j < n; j++)
                {
                    if(arr[j] - arr[j - 1] <= 2)toRight++;
                    else break;
                }
                for(int j = i - 1; j >= 0; j--)
                {
                    if(arr[j + 1] - arr[j] <= 2)toLeft++;
                    else break;
                }
                couldInfect = toLeft + toRight;
                couldInfectArr[i] = couldInfect + 1;
            }
            Arrays.sort(couldInfectArr);
            hp.println(couldInfectArr[0] + " " + couldInfectArr[n - 1]);
        }
        hp.flush();
    }
